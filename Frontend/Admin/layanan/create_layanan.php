...
// base dashboard path dinamis (ngikut project: cleanify2/cleanify5, dll)
$dash = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/') . '/dashboard.php';

// submit harus balik ke route dashboard (karena file ini di-include)
$action = "dashboard.php?page=layanan_create";
...
if (isset($_POST['submit'])) {
    ...
    // name(s), category(s), price(d), duration(i), status(s), short(s), full(s), features(s), not_included(s)
    $stmt->bind_param(
        "ssdisssss",
        $name,
        $category,
...
<form method="POST" action="<?= htmlspecialchars($action) ?>">
    ...
    <a href="dashboard.php?page=layanan" class="btn-secondary">Kembali</a>
</form>
